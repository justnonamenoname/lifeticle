...
формирование квалиа изображения из распределённой информации, отдалённой расстоянием, было бы нарушением принципа локальности.
...
В связи с тем, что квалиа основано на квантовой информации, может оказаться, что напрямую квалиа невозможно обнаружить.
Если квалиа состоит из кадров, то одномоментность кадров также добавляет проблему обнаружению.  
Эксперименты по квантовой физике проходят в изолированной среде, неизвестно существует ли в них квалиа в отличие от среды мозга.  
Распределённая информация, отдалённая расстоянием, не связана и не может быть источником квалиа, то, чтобы разобраться, необходимо изучение схождения сигналов для квалиа.
...
Теория феноменального сознания должна отвечать на эти вопросы, а ответы на них ведут к теории лайфтикла.
...
Размер поля зависит от того, как передаётся информация.
...
1-й метод генерации квалиа не решает данный вопрос.
...
Генератор включает в себя передачу информации на квантовый уровень.  
...
Решением проблемы декодера на стороне лайфтикла было бы существование квалиа объекта без лайфтикла, в этом случае объект, представляющий из себя квалиа, существует, но никем не ощущается.
...
Если квалиа строится постепенно, а кадр феноменального сознания вызывается коллапсом, это даст основания считать, что квалиа-объект может существовать отдельно.
При этом только информация не могла бы создавать боль/отрицательные/положительные квалиа, она должна задаваться противоположностями, например, спином.
Сила боли должна зависеть от количества сигналов, иначе можно было бы задавать любой уровень боли только параметром.
То, как кодируется информация для квалиа будет изучено при изучении генератора квалиа.
Т.к. размер точки зрительного образа ненулевой, это значит мы напрямую наблюдаем волновые функции.
